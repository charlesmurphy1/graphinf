...
TEST_F(NestedBlockPriorTest, getBlockOfIdx_forSomeVertexAtAllLevels_returnCorrectIndex){
    prior.sample();
...
TEST_F(NestedBlockUniformHyperPriorTest, getLogLikelihood_returnSomeOfLogLikelihoodAtEachLevel) {
    double actualLogLikelihood = prior.getLogLikelihood();
    double expectedLogLikelihood = 0;
    for (Level l=0; l<prior.getDepth(); ++l)
        expectedLogLikelihood += prior.getLogLikelihoodAtLevel(l);
