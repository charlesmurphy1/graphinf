...
namespace GraphInf
{
    ...
    public:
        VertexLabelReconstructionMCMC(
            ...
            double betaPrior = 1) : MCMC(betaLikelihood, betaPrior)
        {
            m_labelCallBacks.setMCMC(*this);
            setGraphPrior(graphPrior);
        }
        VertexLabelReconstructionMCMC(
            double betaLikelihood = 1,
            double betaPrior = 1) : MCMC(betaLikelihood, betaPrior) { m_labelCallBacks.setMCMC(*this); }
        ...
        {
            MCMC::reset();
            m_labelCallBacks.clear();
        }
        void onBegin() override
        {
            MCMC::onBegin();
            m_labelCallBacks.onBegin();
        }
        void onEnd() override
        {
            MCMC::onEnd();
            m_labelCallBacks.onEnd();
        }
        void onSweepBegin() override
        {
            MCMC::onSweepBegin();
            m_labelCallBacks.onSweepBegin();
        }
        ...
        {
            MCMC::onStepBegin();
            m_labelCallBacks.onStepBegin();
        }
        void onStepEnd() override
        {
            MCMC::onStepEnd();
            m_labelCallBacks.onStepEnd();
        }
        ...
        {
            return MCMC::isSafe() and (m_graphPriorPtr != nullptr) and (m_graphPriorPtr->isSafe());
        }
